...
앱을 지우거나 폰을 바꾸면 운동 기록도 함께 사라진다. **기록 탭 오른쪽 위 백업 버튼**에서
전부 파일 하나로 뽑아 둘 수 있다. 형식은 [`docs/07-BACKUP.md`](docs/07-BACKUP.md).

**내보내기** — `운동기록_yyyyMMdd.json`이 공유 시트로 나간다.
종목 마스터 · 루틴 전부 · 끝낸 운동 기록이 전부 들어간다(진행 중인 운동은 빠진다).

**복원** — 파일을 고르거나 다른 앱에서 공유하면 미리보기가 뜨고, 두 가지 중에 고른다.

| | |
|---|---|
| **합치기** | 지금 있는 기록을 그대로 두고 빠진 것만 채운다. 겹치는 기록은 건너뛴다 |
| **덮어쓰기** | 지금 있는 것을 모두 지우고 백업 내용만 남긴다. 확인 다이얼로그를 한 번 더 거친다 |

폰을 새로 깔고 복원하는 경우 **합치기**면 충분하다 — 시드 루틴은 이름이 같아 그대로 재사용되고,
기록만 들어온다. 같은 파일을 두 번 넣어도 기록이 겹치지 않는다(세션의 신원은 시작 시각).

복원은 통째로 한 트랜잭션이다. 중간에 실패하면 아무것도 바뀌지 않는다.
...
| [`docs/05-IOS.md`](docs/05-IOS.md) | iOS 빌드에서 미리 해둔 설정과 맥에서만 할 수 있는 일 |
| [`docs/06-ANDROID-SIGNING.md`](docs/06-ANDROID-SIGNING.md) | 서명 키를 옮기는 법. 안 옮기면 기존 설치본의 기록이 끊긴다 |
| [`docs/07-BACKUP.md`](docs/07-BACKUP.md) | 기록 백업 `.json` 포맷과 복원 규칙 |
...
기록은 이 폰의 SQLite 파일에만 있다. 서버도 계정도 없다. **앱을 지우면 같이 사라지므로**
가끔 백업 파일을 뽑아 두는 것이 유일한 안전망이다(위 "기록 백업 / 복원").
