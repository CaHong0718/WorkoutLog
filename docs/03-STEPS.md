...
- [ ] `HomeBloc` — Intent: `Load`, `SelectDay`, `StartSession`, `ResumeSession`
- [ ] 오늘의 DAY 카드 (코드/제목/총 세트/예상 시간, 부위 색상 적용)
- [ ] 블록별 종목 미리보기 (슈퍼세트 묶음 시각화)
- [ ] DAY 선택 시트 (순번 자동 + 수동 변경)
- [ ] 진행 중 세션이 있으면 "이어서 하기" 배너
- [ ] 이번 주 볼륨 요약 (부위별 막대, HTML의 vchart 스타일)
- [ ] 하단 네비게이션 (홈 / 기록 / 루틴)
