...
## STEP 15 — 같은 종목 값 이어받기

**실사용 피드백.** "같은 종목인데 세트마다 무게·횟수를 다시 입력하는 게 너무 귀찮다."

원인은 미리 채우기의 출처가 **지난 세션뿐**이었다는 것. 오늘 1세트를 60kg로 올렸어도
2세트 칸에는 지난주의 55kg가 들어와, 매 세트 손으로 고쳐야 했다.

- [x] `SessionState.carryOverFor(planned)` — 이번 세션에서 **같은 종목으로 이미 끝낸
      가장 가까운 앞 세트**를 찾는다
- [x] `CurrentSetCard.carryOver` — 미리 채우기 우선순위에 두 번째로 끼운다
- [x] `test/session_test.dart` 5개 추가 (상태 계산) · `test/session_widget_test.dart` 5개 신설
      (입력 칸에 실제로 무엇이 들어가는지)

**미리 채우기 우선순위** — 앞의 것이 이길수록 "지금 무슨 일이 벌어지는지"를 잘 안다.

| 순위 | 출처 | 언제 |
|---|---|---|
| 1 | 이 세트 자신의 기록 (`existingLog`) | 되돌아가 고치는 중 — 남의 값을 보여주면 안 된다 |
| 2 | **이번 세션의 같은 종목 직전 세트** (`carryOver`) | 바에 그 무게가 그대로 걸려 있다 |
| 3 | 지난 세션의 같은 번호 세트 | 오늘 아직 이 종목을 안 했다 |
| 4 | 지난 세션의 아무 세트 → `repMin` | 그것도 없다 |

정한 규칙 네 가지:

| 갈림길 | 정한 것 | 이유 |
|---|---|---|
| 무엇으로 짝을 맞추나 | 루틴 항목이 아니라 **종목 id** | 세션 대체(`SubstituteExercise`)로 기구가 바뀌면 앞 종목의 무게가 따라오면 안 된다 |
| 어디까지 훑나 | 계획 순서로 **앞선 세트만** (`itemOrder`, `setIndex`) | 앞으로 점프해 기록한 뒤 돌아왔을 때 뒷 세트 값이 앞으로 새면 안 된다 |
| 건너뛴 세트 | 제외 (`isCompleted`) | 숫자가 없는 행이다 |
| RIR | **이어받지 않는다** — 루틴이 처방한 `targetRir` 유지 | 세트가 쌓일수록 실제로 내려간다. 앞 세트 값을 그대로 기록에 남기는 게 더 나쁘다 |

`didUpdateWidget`은 `carryOver` 변화를 일부러 무시한다. 타임라인에서 다른 세트를 고치면
`carryOver`가 움직이는데, 그때 다시 채우면 **입력 중이던 숫자가 지워진다.** 세트마다 카드는
새 `key`로 다시 만들어지므로 무시해도 값은 제때 갱신된다.

**완료 조건**: `flutter analyze` 무경고 · `flutter test` 172개 통과.
